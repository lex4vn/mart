...
(function (factory) {
  ...
  var Editor = function () {
    ...
    this.unlink = function ($editable) {
       ...
       * @param {Object} options
       */
      this.createImageInfo = function ($editable, imageInfo, options) {
          var imageAlt = imageInfo.alt;
          var imageTitle = imageInfo.title;

          console.log("Alt"+imageInfo.alt);
          var isNewWindow = imageInfo.newWindow;
          console.log("newWindow"+imageInfo.newWindow);
          var rng = imageInfo.range;
          console.log("range"+imageInfo.range);
          recordUndo($editable);
          var src = imageInfo.src;
          console.log("src"+imageInfo.src);
         // imageAlt = options.onCreateLink(imageAlt);

          rng = rng.deleteContents();
//          console.log("rng"+rng);
//          // Create a new link when there is no anchor on range.
          var image = rng.insertNode('<IMG></IMG>');
//          $(image).attr({
//              alt: imageAlt,
//              title: imageTitle
//          });

      //    range.createFromNode(image).select();
          range.create().insertNode(image);
          triggerOnChange($editable);
      };
    /**
     * create link
    ...
    this.createLink = function ($editable, linkInfo, options) {
      ...
      this.getImageInfo = function ($editable) {
          ...
          var rng = range.create().expand(dom.isImg);
          ...
          return {
              ...
              isNewWindow: $img.length ? $img.attr('target') === '_blank' : true,
              alt: $img.attr('alt')
          };
      };
    /**
      ...
      this.showImageEditDialog = function ($editable, $dialog, imageInfo) {
          return $.Deferred(function (deferred) {
              ...
              var $imageAlt = $imageEditDialog.find('.note-image-alt'),
                  ...
                  $openInNewWindow = $imageEditDialog.find('input[type=checkbox]');

              $imageEditDialog.one('shown.bs.modal', function () {
                  $imageAlt.val(imageInfo.alt);

                  $imageAlt.keyup(function () {
                      // if linktext was modified by keyup,
                      // stop cloning text from linkUrl
                      imageInfo.alt = $imageAlt.val();
                  });

                  // if no url was given, copy text to url
                  if (!imageInfo.alt) {
                      imageInfo.alt = imageInfo.text;
                      toggleBtn($editBtn, imageInfo.text);
                  }

                  $imageTitle.keyup(function () {
                      toggleBtn($editBtn, $imageTitle.val());
                      // display same link on `Text to display` input
                      // when create a new link
                      if (!imageInfo.text) {
                          $imageTitle.val($imageTitle.val());
                      }
                  }).val(imageInfo.title).trigger('focus').trigger('select');

                  $openInNewWindow.prop('checked', imageInfo.newWindow);
      ...
      showLinkDialog: function (layoutInfo) {
        ...
        showImageEditDialog: function (layoutInfo) {
            ...
            dialog.showImageEditDialog($editable, $dialog, imageInfo).then(function (imageInfo) {
                editor.restoreRange($editable);
                editor.createImageInfo($editable, imageInfo, options);
                // hide popover after creating link
                popover.hide(layoutInfo.popover());
            }).fail(function () {
                editor.restoreRange($editable);
    ...
    var tplDialogs = function (lang, options) {
      ...
      var tplLinkDialog = function () {
        ...
        var tplImageEditDialog = function () {
            ...
            var footer = '<button href="#" class="btn btn-primary note-image-edit-btn disabled" disabled>Cập nhật</button>';
