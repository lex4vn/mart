...
public class ArchiveUtils {
    ...
    public static final void unzipArchive(File zip, File extractTo) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("Failed to unzip archive %s to %s", zip.getAbsolutePath(), extractTo.getAbsolutePath()), e);
        } finally {
    ...
    /**
     * Extracts archive to target directory. Supported archive type: .zip, .tar, .jar, .ar, 7z, ...
     * 
    ...
    public static void extract(File archive, File targetDir) {
        ...
        try {
            input =
                new ArchiveStreamFactory().createArchiveInputStream(new BufferedInputStream(new FileInputStream(archive)));
            FDLogger.debug("Archive file class " + input.getClass());
            ...
            FDLogger.debug("Extracted " + archive.getAbsolutePath() + " to " + targetDir.getAbsolutePath()
                + " successfully");
     ...
     * @param output decompressed file
     * @throws RuntimeException
     */
    public static void decompress(File compressedFile, File output) {
        CompressorInputStream is = null;
        try {
            is = new CompressorStreamFactory().createCompressorInputStream(new BufferedInputStream(new FileInputStream(compressedFile)));
            FDLogger.debug("Compressed file class " + is.getClass());
            FileUtils.copyInputStreamToFile(is, output);
            FDLogger.debug("Decompressed " + compressedFile.getAbsolutePath() + " to " + output.getAbsolutePath()
                + " successfully");
    ...
    /**
     * Decompresses and then extract file. This method applicable for such filetype like tar.gz or
     * tar.bz2.
     * 
     * @param compressedFile
     * @param targetDir
     * @throws RuntimeException if anything goes wrong while decompressing file
     */
    ...
    public static void archive(File srcDir, File dstArchive, ArchiveType type) {
        ...
        try {
            aOS =
                new ArchiveStreamFactory().createArchiveOutputStream(aType, new BufferedOutputStream(new FileOutputStream(dstArchive)));
     ...
     * @param aOS
     * @param fileToAdd
     * @param base base prefix to for the name of the archive file entry
     * @param archiveType zip or tar
     * @throws RuntimeException
     */
    ...
    public static void compress(File srcFile, File dstFile, ArchiveType type) {
        ...
        try {
            cOS =
                new CompressorStreamFactory().createCompressorOutputStream(aType, new BufferedOutputStream(new FileOutputStream(dstFile)));
